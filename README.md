...
```
docker pull sequenceiq/spark:1.2.0-ubuntu
```

## Building the image
```
docker build --rm -t sequenceiq/spark:1.2.0-ubuntu .
```

## Running the image
```
docker run -i -t -h sandbox sequenceiq/spark:1.2.0-ubuntu /etc/bootstrap.sh -bash
```

## Versions
```
Ubuntu 14.04 Hadoop 2.6.0 and Apache Spark v1.2.0
```
